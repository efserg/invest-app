...
class StockServiceImpl(dao: StockDao) extends StockService {
  override def get(request: GetStockRequest): GetStockResponse =
    dao.get(request.ticker) match {
      ...
      case Failure(ex) =>
        ex match {
          case _: StockNotFoundException =>
            GetStockResponse.NotFound(ex.getMessage)
  ...
  override def delete(request: DeleteStockRequest): DeleteStockResponse =
    dao.delete(request.ticker) match {
      ...
      case Failure(ex) =>
        ex match {
          case _: StockNotFoundException =>
            DeleteStockResponse.NotFound(ex.getMessage)
          case _ => DeleteStockResponse.Failed(ex.getMessage)
        }
    }

  override def update(request: UpdateStockRequest): UpdateStockResponse =
    dao.update(request.ticker, Stock(request.ticker, request.currency)) match {
      case Success(s) => UpdateStockResponse.Updated(s)
      case Failure(ex) =>
        ex match {
          case _: StockNotFoundException =>
            UpdateStockResponse.NotFound(ex.getMessage)
          case _: StockValidateException =>
            UpdateStockResponse.NotValid(ex.getMessage)
          case _ => UpdateStockResponse.Failed(ex.getMessage)
        }
    }

  override def create(request: CreateStockRequest): CreateStockResponse =
    dao.create(Stock(request.ticker, request.currency)) match {
      case Success(s) => CreateStockResponse.Created(s)
      case Failure(ex) =>
        ex match {
          case _: StockAlreadyExistException =>
            CreateStockResponse.AlreadyExist(ex.getMessage)
          case _: StockValidateException =>
            CreateStockResponse.NotValid(ex.getMessage)
